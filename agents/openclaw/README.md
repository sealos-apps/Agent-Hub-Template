...
- Agent Hub test image: `ghcr.io/gitlayzer/openclaw:2026.5.19-agenthub-test.3`
...
Agent Hub 场景下同一个 Devbox 会通过平台鉴权和网关 token 控制访问，镜像默认写入 `gateway.controlUi.dangerouslyDisableDeviceAuth=true`，关闭 OpenClaw Control UI 的设备配对流程，避免公网域名打开 dashboard 时要求在 Gateway 主机执行 `openclaw devices approve ...`。
